...
public class UsuarioController {
    ...
    public ResponseEntity<Long> cadastrarFornecedor(@RequestBody UsuarioRequest request) {
        Usuario usuario = service.cadastrarFornecedor(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(usuario.getId());
    }


    @PostMapping("/fornecedor/login")
    public ResponseEntity<String> loginFornecedor(@RequestBody LoginRequest request) {
        Usuario usuario = this.service.autenticar(request);
        String token = jwtService.gerarToken(usuario);
        return ResponseEntity.ok(token);
    }
